//
//  DeclineHabitInvitationUseCase.swift
//  StickTogetherIOS
//
...
protocol DeclineHabitInvitationUseCase {
    func execute(notificationId: String, habit: Habit, currentUserId: String) async throws
}

final class DeclineHabitInvitationUseCaseImpl: DeclineHabitInvitationUseCase {
    ...
    func execute(notificationId: String, habit: Habit, currentUserId: String) async throws {
        ...
        try await transactionRepository.run { [weak self] ctx in
            ...
            try habitRepository.updateHabitFields(
                ...
                fields: ["invitedBuddyIds": .remove([currentUserId])],
                habitId: habitId
            )
            let notification = Notification(
                ...
                title: habit.title,
                body: "\(currentUser.name) declined your invitation to \"\(habit.title)\"",
                type: .systemMessage
            )
